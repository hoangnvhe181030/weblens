...
export function PageDetailPage() {
  ...
  async function startCapture() {
    if (!page || submittingCapture) return
  ...
  return (
    <div className="app-page">
      ...
      <header className="detail-header"><div><span className="page-kicker">SCAN PAGE EVIDENCE</span><h1>{page.path}</h1><a href={page.url} target="_blank" rel="noreferrer">{page.url}<ExternalLink size={14} /></a></div><button className="button button--primary" type="button" disabled={submittingCapture || Boolean(capture && !terminalCaptureStatuses.has(capture.status))} onClick={startCapture}>{submittingCapture || (capture && !terminalCaptureStatuses.has(capture.status)) ? <LoaderCircle className="spin" /> : <Camera />}{submittingCapture ? 'Đang tạo…' : capture?.status === 'RUNNING' ? 'Chromium đang render…' : capture?.status === 'INDEXING' ? 'Đang lập chỉ mục…' : 'Tạo browser capture'}</button></header>
      ...
      <section className="evidence-metrics"><article><span><Code2 /></span><small>HTTP STATUS</small><strong>{page.statusCode ?? '—'}</strong><p>{page.outcome === 'failed' ? 'Không thành công' : 'Phản hồi nhận được'}</p></article><article><span><Clock3 /></span><small>RESPONSE TIME</small><strong>{page.responseTimeMs !== undefined ? `${page.responseTimeMs} ms` : '—'}</strong><p>TTFB {page.timing?.ttfbMillis !== undefined ? `${page.timing.ttfbMillis} ms` : 'không phát sinh phép đo'}</p></article><article><span><FileCode2 /></span><small>HTML SIZE</small><strong>{page.responseBytes !== undefined ? formatBytes(page.responseBytes) : '—'}</strong><p>Ngưỡng cảnh báo: 293 KB</p></article></section>
