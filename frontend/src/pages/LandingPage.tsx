    ...
    <div className="marketing-page">
      ...
      <header className="marketing-header">
        <div className="container nav-wrap">
          ...
          <div className="nav-actions"><Link className="text-link" to="/login">Đăng nhập</Link><Link className="button button--dark button--small" to="/register">Bắt đầu demo <ArrowRight size={15} /></Link></div>
      ...
      <main id="main-content">
        <section className="hero-section">
          ...
          <div className="container hero-grid">
            <div className="hero-copy reveal">
              ...
              <form className="url-form" onSubmit={submitDemo} noValidate>
                <label htmlFor="hero-url">Website bạn muốn quan sát</label>
                <div className="url-input-wrap"><ScanLine aria-hidden="true" /><input id="hero-url" value={url} onChange={(event) => { setUrl(event.target.value); if (error) setError(null) }} aria-invalid={Boolean(error)} aria-describedby={error ? 'hero-url-error' : 'hero-url-help'} /><button type="submit">Quét thử <ArrowRight size={17} /></button></div>
                {error ? <p className="field-error" id="hero-url-error" role="alert">{error}</p> : <p className="form-help" id="hero-url-help"><Check size={14} aria-hidden="true" /> Demo miễn phí · Không cần thẻ · Không gửi request thật</p>}
              </form>
            </div>
            <div className="hero-visual reveal reveal--delay"><div className="orbit orbit--one" aria-hidden="true" /><div className="orbit orbit--two" aria-hidden="true" /><ScanVisual /><div className="floating-note floating-note--top"><Zap size={15} /> Tiến độ realtime</div><div className="floating-note floating-note--bottom"><ShieldCheck size={15} /> Giới hạn an toàn</div></div>
          </div>
        ...
        <section className="section section--ink" id="platform">
          <div className="container">
            ...
            <div className="feature-grid">{features.map(({ icon: Icon, eyebrow, title, text, accent }) => <article className={`feature-card feature-card--${accent}`} key={title}><div className="feature-icon"><Icon aria-hidden="true" /></div><small>{eyebrow}</small><h3>{title}</h3><p>{text}</p><Link to="/app/scans/scan-103">Xem trong dashboard <ChevronRight size={16} /></Link></article>)}</div>
          </div>
        </section>
        ...
        <section className="section evidence-section">
          <div className="container split-layout">
            <div className="evidence-board">
              <div className="board-header"><span><i className="pulse" /> SCAN ĐANG CHẠY</span><strong>12 / 18 trang</strong></div>
        ...
        <section className="section capture-section" id="capture">
          <div className="container">
            ...
            <div className="capture-layout">
              <div className="browser-frame"><div className="browser-bar"><span className="window-dots"><i /><i /><i /></span><span>evomi.com</span><Camera size={15} /></div><div className="mock-site"><div className="mock-nav"><span /><span /><span /></div><div className="mock-title"><i /><i /></div><div className="mock-cards"><span /><span /><span /></div><div className="scan-overlay"><small>SNAPSHOT #1</small><strong>Đã ghi lại 126 tài nguyên</strong><div><span>DOM cuối</span><span>Ảnh chụp</span><span>Network</span></div></div></div></div>
        ...
        <section className="cta-section"><div className="cta-grid" aria-hidden="true" /><div className="container cta-content"><Sparkles aria-hidden="true" /><h2>Bắt đầu với một URL.<br />Kết thúc bằng bằng chứng.</h2><p>Khám phá toàn bộ luồng WebLens bằng dữ liệu demo an toàn.</p><Link className="button button--light" to="/register">Mở dashboard demo <ArrowRight /></Link></div></section>
      </main>

      <footer className="marketing-footer"><div className="container footer-top"><Brand inverse /><p>Nền tảng website intelligence dựa trên bằng chứng, dành cho developer.</p><div><Link to="/app/websites">Dashboard</Link><a href="#platform">Nền tảng</a><a href="#roadmap">Roadmap</a></div></div><div className="container footer-bottom"><span>© 2026 WebLens · Frontend demonstration</span><span>Không thực hiện request scan thật</span></div></footer>
    </div>
  )
}
